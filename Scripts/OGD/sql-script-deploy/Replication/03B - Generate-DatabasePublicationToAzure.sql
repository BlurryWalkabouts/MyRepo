...
*******************************************************************************************************************
 ...
 @replicationAccount      = local account used to access the source database (repl_ogd_<application>)
 @replicationPassword     = Password of the source access account
...
*******************************************************************************************************************
 * DistributionAgentAccount must have "create new files" permission on folder
 ...
 * SnapshotLocation must exist (including share):
   * SourceAgentAccount must have RW permissions
   * DistributionAgentAccount must have R permissions
*******************************************************************************************************************/
DECLARE @customerNumber nchar(6) = '001013';
...
DECLARE @sourceDomain nvarchar(255) = '';
 ...
 *************************************************************************************
 @sourceAgentAccount        = Accountname needed for replication
 @distributionAgentAccount  = Accountname needed for replication
 ...
 *************************************************************************************/
DECLARE @sourceAgentAccount nvarchar(255) = CONCAT(@sourceServerName, '\', @replicationAccount);
DECLARE @distributionAgentAccount nvarchar(255) = CONCAT(@distributionServerName, '\', @replicationAccount);
...
SET @sql += 'EXEC sp_addlogreader_agent @job_login = N''' + @sourceAgentAccount + ''', @job_password = ''' + @replicationPassword + ''', @publisher_security_mode = 1;' + char(10);
...
SET @sql += 'EXEC sp_addpushsubscription_agent @publication = N''' + @publication + ''', @subscriber = N''' + @replicaServerName + ''', @subscriber_db = N''' + @replicaDatabaseName + ''', @subscriber_security_mode = 0, @subscriber_login = N''' + @sqlPushAccount + ''', @subscriber_password = N''' + @sqlPushPassword + ''', @job_login = N''' + @sourceAgentAccount + ''', @job_password = ''' + @replicationPassword + ''';' + char(10);
