CREATE FUNCTION [Security].[fn_rlsGeneralSecurityPredicate](@CustomerKey int, @OperatorGroupKey int, @ToggleOperatorGroup bit)  
    RETURNS TABLE  
    --WITH SCHEMABINDING  
AS  
    RETURN 
		...
		inner JOIN [Security].[AzureGroup] G ON (G.id = AGM.azureGroupId AND IsMember=1)
		WHERE 
		(
			1=1
