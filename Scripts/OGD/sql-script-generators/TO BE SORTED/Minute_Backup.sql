...
/****** Object:  Job [00 - Backup - Minutely Log]    Script Date: 5/1/2017 8:12:06 PM ******/
EXEC msdb.dbo.sp_delete_job @job_id=N'e586d2da-772b-4800-a9bf-2fa86b2e96a2', @delete_unused_schedule=1
GO
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 5/1/2017 8:12:06 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'00 - Backup - Minutely Log', 
		@enabled=1, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Log Backup]    Script Date: 5/1/2017 8:12:06 PM ******/
...
BEGIN   
	   SET @fileName = @path + @name + ''_'' + REPLACE (REPLACE (REPLACE (CONVERT (VARCHAR (40), GETDATE (), 120), ''-'',''''),'':'', ''''),'' '', '''') + ''.trn''
	   SET @mediaName = @name + ''- Log Backup''
       BACKUP LOG @name 
		TO URL = @fileName
		WITH 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Minute Schedule', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
		...
		@schedule_uid=N'8686b7f9-a4e0-45a3-900a-b206994b6ea7'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
