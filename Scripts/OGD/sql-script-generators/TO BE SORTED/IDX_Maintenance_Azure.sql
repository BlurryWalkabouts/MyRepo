...
if object_id('DBA.AzureSQLMaintenance') is null
	exec('create procedure DBA.AzureSQLMaintenance as /*dummy procedure body*/ select 1;')	
GO
ALTER Procedure [DBA].[AzureSQLMaintenance]
	(
...
begin
	...
	/* Prepare Log Table */
	if object_id('DBA.AzureSQLMaintenanceLog') is null 
		begin
			create table [DBA].[AzureSQLMaintenanceLog] (id bigint primary key identity(1,1), OperationTime datetime2, command varchar(4000),ExtraInfo varchar(4000), StartTime datetime2, EndTime datetime2, StatusMessage varchar(1000));
		end

	if @LogToTable=1 insert into [DBA].[AzureSQLMaintenanceLog] values(@OperationTime,null,null,sysdatetime(),sysdatetime(),'Starting operation: Operation=' +@operation + ' Mode=' + @mode + ' Keep log for last ' + cast(@KeepXOperationInLog as varchar(10)) + ' operations' )	
	...
	begin 
		...
		begin
			...
			if @LogToTable=1 insert into [DBA].[AzureSQLMaintenanceLog] values(@OperationTime,@SQLCMD,@ExtraInfo,sysdatetime(),null,'Started')
			begin try
				exec(@SQLCMD)	
				if @LogToTable=1 update [DBA].[AzureSQLMaintenanceLog] set EndTime = sysdatetime(), StatusMessage = 'Succeeded' where id=SCOPE_IDENTITY()
			end try
			begin catch
				raiserror('cached',0,0) with nowait
				if @LogToTable=1 update [DBA].[AzureSQLMaintenanceLog] set EndTime = sysdatetime(), StatusMessage = 'FAILED : ' + CAST(ERROR_NUMBER() AS VARCHAR(50)) + ERROR_MESSAGE() where id=SCOPE_IDENTITY()
			end catch
			delete from @T
		end
	end
	...
	if @LogToTable=1
	begin
		delete from [DBA].[AzureSQLMaintenanceLog] 
		from 
			[DBA].[AzureSQLMaintenanceLog] L join 
			(select distinct OperationTime from [DBA].[AzureSQLMaintenanceLog] order by OperationTime desc offset @KeepXOperationInLog rows) F
				ON L.OperationTime = F.OperationTime
		insert into [DBA].[AzureSQLMaintenanceLog] values(@OperationTime,null,cast(@@rowcount as varchar(100))+ ' rows purged from log table because number of operations to keep is set to: ' + cast( @KeepXOperationInLog as varchar(100)),sysdatetime(),sysdatetime(),'Cleanup Log Table')
	end

	raiserror('Done',0,0)
	if @LogToTable=1 insert into [DBA].[AzureSQLMaintenanceLog] values(@OperationTime,null,null,sysdatetime(),sysdatetime(),'End of operation')
end
GO
