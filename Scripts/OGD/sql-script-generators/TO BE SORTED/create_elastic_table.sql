DECLARE @schemaName nvarchar(255) = 'MasterData'
...
declare T cursor  for
	...
	WHERE table_schema IN ('MasterData') -- and table_name = 'customer'
...
'smallint', 'uniqueidentifier', 'money', 'time', 'date') THEN '' WHEN DATA_Type IN ('datetime2','datetime') THEN '(' + CAST(datetime_precision AS NVARCHAR) + ') ' ELSE '(' + CAST(NUMERIC_PRECISION AS NVARCHAR) + ',' + CAST(NUMERIC_SCALE AS NVARCHAR) + ')'  END) END) + (CASE WHEN IS_NULLABLE = 'YES' THEN  'NULL' ELSE ' NOT NULL' END) as nvarchar(max)) 
