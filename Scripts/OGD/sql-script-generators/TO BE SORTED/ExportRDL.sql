...
DECLARE @OutputPath AS VARCHAR(500) = 'F:\Reports\' 
...
BEGIN 
   ...
   */ 
   SET @TSQL = STUFF((SELECT
                      ...
                      '       END) ' + 
                      ' FROM ' + 
                      ' [ReportDWH2016].[dbo].[Catalog] CL ' + 
                      ' CROSS APPLY (SELECT CONVERT(VARBINARY(MAX),CL.Content) Content) C ' + 
                      ' WHERE ' + 
                      ' CL.ItemID = ''''' + CONVERT(VARCHAR(MAX), CL.ItemID) + ''''' " queryout "' + @OutputPath + '' + CL.Name + '.rdl" ' + '-T -c -x  -S(local)\Acceptance'''
                    FROM 
                      [ReportDWH2016].[dbo].[Catalog] CL 
                    WHERE 
                      CL.[Type] = 2 --Report 
