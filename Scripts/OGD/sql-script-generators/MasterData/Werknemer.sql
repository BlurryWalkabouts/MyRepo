...
select distinct
	...
	,[FunctionLevel_Code] = S.unid
	,[FunctionLevel_Name] = S.tekst
	 -- People responsible for employee
	,[Manager_Code] = CASE WHEN M.tekst = 'Geen' THEN NULL ELSE M1.unid END
	,[Manager_Name] = M.tekst
	,[CareerAdvisor_Code] = HR1.unid
	,[CareerAdvisor_Name] = CASE WHEN HR1.unid IS NULL THEN NULL ELSE HR.tekst END
	 -- Various
	...
	,[ContractAvailabilityPercentage] = COALESCE(CAST(wc.procent AS INT), 0)
...
*/
select distinct team_name, function_name from R order by team_name, function_name
--where businessunit != '[Not Available]' AND Team != '[Not Available]'
--where [function] = 'Teamleider SSD Helvetios'
--where lastname LIKE 'Dus%'
--where team = 'mt'
