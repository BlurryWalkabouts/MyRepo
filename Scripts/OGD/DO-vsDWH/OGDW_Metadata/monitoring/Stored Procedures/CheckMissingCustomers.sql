CREATE PROCEDURE [monitoring].[CheckMissingCustomers]
(
	@sendmail bit = 0
)
AS
...
IF @sendmail = 1
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM monitoring.MissingCustomers)
	BEGIN
		-- Als er rijen worden gevonden, wordt de body van het mailbericht opgebouwd als HTML
		DECLARE @subject nvarchar(64) = 'Missing Customers'
...
END
ELSE
	SELECT * FROM monitoring.MissingCustomers
