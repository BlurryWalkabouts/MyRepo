CREATE PROCEDURE [monitoring].[CheckMissingAnywhereMappings]
(
	@sendmail bit = 0
)
AS
...
IF @sendmail = 1
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM monitoring.MissingAnywhereMappings)
	BEGIN
		-- Als er rijen worden gevonden, wordt de body van het mailbericht opgebouwd als HTML
		DECLARE @subject nvarchar(64) = 'Missing Anywhere Mappings'
		...
		<table cellpadding="5" cellspacing="0" border="1">
			...
			SELECT
				td = skillChosen
			FROM
				monitoring.MissingAnywhereMappings
...
END
ELSE
	SELECT * FROM monitoring.MissingAnywhereMappings
