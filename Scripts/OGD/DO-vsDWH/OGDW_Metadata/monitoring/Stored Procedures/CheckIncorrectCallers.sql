CREATE PROCEDURE [monitoring].[CheckIncorrectCallers]
(
	@sendmail bit = 0
)
AS
...
IF @sendmail = 1
BEGIN
	IF EXISTS (SELECT TOP 1 1 FROM monitoring.IncorrectCallers)
	BEGIN
		-- Als er rijen worden gevonden, wordt de body van het mailbericht opgebouwd als HTML
		DECLARE @subject nvarchar(64) = 'Incorrect Callers'
		...
		<table cellpadding="5" cellspacing="0" border="1">
			...
			SELECT
				...
				, td = COALESCE(CallerName,'NULL')
			FROM
				monitoring.IncorrectCallers
...
END
ELSE
	SELECT * FROM monitoring.IncorrectCallers
