...
CREATE PROCEDURE [dbo].[Inc_FirstLineFix_per_Interval_Interfood_v01]
...
/* Aantal afgemelde meldingen bij SAM */
, IncClosedSAM AS
(
SELECT 
	ClosureDate
	, AantalGeslotenSAM = COUNT(Incident_Id)
FROM
	#FilteredIncidents
WHERE 1=1
	AND OperatorGroup IN('APPLICATION MANAGEMENT','SYSTEM MANAGEMENT')
	AND ClosureDate IS NOT NULL
GROUP BY
	ClosureDate
)
 
--SELECT * FROM IncClosedSAM ORDER BY ClosureDate
...
SELECT
	...
	, AfgemeldSAM = ISNULL(AantalGeslotenSAM,0)
	, AfgemeldOverig = ISNULL(AantalGesloten,0) - ISNULL(AantalGeslotenSD,0) - ISNULL(AantalGeslotenSAM,0)
FROM
	Dim.[Date] D
	LEFT OUTER JOIN IncClosed Cl ON D.[Date] = Cl.ClosureDate
	LEFT OUTER JOIN IncClosedSD Cl2 ON D.[Date] = Cl2.ClosureDate
	LEFT OUTER JOIN IncClosedSAM Cl3 ON D.[Date] = Cl3.ClosureDate
WHERE 1=1
...
SELECT
	...
	, IntervalString = CASE @ReportInterval
			...
			WHEN 'Day' THEN CONVERT(nvarchar(12),[Date],5)
		END
	, AfgemeldTotaal = SUM(AfgemeldTotaal)
	, AfgemeldSD = SUM(AfgemeldSD)
	, AfgemeldSAM = SUM(AfgemeldSAM)
	, AfgemeldOverig = SUM(AfgemeldOverig)
...
/*
EXEC [dbo].[Inc_FirstLineFix_per_Interval_Interfood_v01]
@Customer = '90'
, @SourceDatabase = '-99'
