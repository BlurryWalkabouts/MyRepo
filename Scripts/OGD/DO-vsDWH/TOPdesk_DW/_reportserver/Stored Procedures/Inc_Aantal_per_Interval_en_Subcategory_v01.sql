...
CREATE PROCEDURE [dbo].[Inc_Aantal_per_Interval_en_Subcategory_v01] 
...
FROM
	...
	LEFT OUTER JOIN IncCreated Cr ON D.[Date] = Cr.IncidentDate AND Cr.Subcategory = S.Subcategory
	LEFT OUTER JOIN IncClosed Cl ON D.[Date] = Cl.ClosureDate AND Cl.Subcategory = S.Subcategory
	LEFT OUTER JOIN IncOpen O ON D.[Date] = O.[Date] AND O.Subcategory = S.Subcategory
	LEFT OUTER JOIN IncGereed G ON D.[Date] = G.[Date] AND G.Subcategory = S.Subcategory
WHERE 1=1
	AND D.[Date] BETWEEN @ReportStartDate AND @ReportEndDate
)
...
/*
EXEC [dbo].[Inc_Aantal_per_Interval_en_Subcategory_v01]
@Customer = '44'
, @SourceDatabase = '-99'
