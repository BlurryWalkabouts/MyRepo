...
CREATE PROCEDURE [dbo].[Inc_Aantal_per_Interval_en_Cat_Subcategory_Top_v01]
...
/* Query om het aantal aangemelde, afgemelde, openstaande en gereedgemelde meldingen
	te bepalen per subcategorie over de laatste maand. Met SELECT TOP 5
...
(
SELECT TOP 5 
	CatAndSubCat = CONCAT(Subcategory,' (',Category,')')
FROM
	#FilteredIncidents
GROUP BY
	CONCAT(Subcategory,' (',Category,')')
ORDER BY
	COUNT(*) DESC -- !!!NAAR DEZE SORTEERVOLGORDE MOET NOG GEKEKEN WORDEN!!! Was 'weggecomment' in originele dataset
)

--SELECT * FROM TopSubCat
...
FROM
	EndResult
WHERE 1=1
	AND CatAndSubCat IN(SELECT * FROM TopSubCat)
ORDER BY
	DWInterval
	, Category
	, Subcategory
...
/*
EXEC [dbo].[Inc_Aantal_per_Interval_en_Cat_Subcategory_Top_v01]
@Customer = '10'
, @SourceDatabase = '-99'
