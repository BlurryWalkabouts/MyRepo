...
CREATE PROCEDURE [dbo].[Inc_Aantal_per_Interval_en_IncidentType_v02] 
...
/* Query om het aantal aangemelde meldingen te bepalen per Incident type
...
WHERE 1=1
	...
	AND IncidentTypeSTD NOT IN('Beveiligingsincident','[Onbekend]','Geen')
...
SELECT
	...
	, TotaalPerIncidentType = SUM(ISNULL(AantalAangemeld,0)) OVER (PARTITION BY IncidentType)
FROM
	Dim.[Date] D
...
SELECT
	...
	, TotaalPerIncidentType = MAX(TotaalPerIncidentType)
FROM
	CountsPerDay
GROUP BY
	CASE @ReportInterval
...
ORDER BY
	DWInterval
	, TotaalPerIncidentType DESC

END

/*
EXEC [dbo].[Inc_Aantal_per_Interval_en_IncidentType_v02]
@Customer = '29'
, @SourceDatabase = '-99'
