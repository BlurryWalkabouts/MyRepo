...
CREATE PROCEDURE [dbo].[Inc_Aantal_per_Interval_en_EntryType_v02] 
...
/* Query om de verdeling tussen soort binnenkomst van meldingen te berekenen per gekozen interval. Te filteren op o.a. status, operatorgroup en category.

	Geschreven door Wouter Gielen, Mark Krijtenberg
	Laatste update 7-6-2016 */
...
WHERE 1=1
	...
	AND EntryTypeSTD NOT IN('[Onbekend]','Monitor','Eigen constatering')
ORDER BY
	Incident_Id
...
SELECT
	...
	, TotaalPerEntryType = SUM(ISNULL(AantalAangemeld,0)) OVER (PARTITION BY EntryType)
	, TotaalPerInterval = SUM(ISNULL(AantalAangemeld,0)) OVER (PARTITION BY CASE @ReportInterval
													WHEN 'Month' THEN CAST(DWMonthNumber AS nvarchar(12))
													WHEN 'Week' THEN CAST(DWWeekNumber AS nvarchar(12))
													WHEN 'Day' THEN CONVERT(nvarchar(12),[DateKey],5)
												END)
...
SELECT
	...
	, TotaalPerEntryType = MAX(TotaalPerEntryType)
	, TotaalPerInterval = MAX(TotaalPerInterval)
	, EntryTypePerc = IIF(MAX(TotaalPerInterval) = 0, 0, SUM(Aangemeld) / CAST(MAX(TotaalPerInterval) AS decimal(9,2)))
FROM
	CountsPerDay
GROUP BY
	CASE @ReportInterval
...
ORDER BY
	DWInterval
	, TotaalPerEntryType DESC

END

/*
EXEC [dbo].[Inc_Aantal_per_Interval_en_EntryType_v02]
@Customer = '10'
, @SourceDatabase = '-99'
