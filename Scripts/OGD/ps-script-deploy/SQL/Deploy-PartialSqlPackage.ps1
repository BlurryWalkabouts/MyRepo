...
try {
    $ExistingSchemas = (Invoke-SqlServerQuery -Verbose @CommonQueryParameters -ErrorAction Stop `
...
$ExistingObjects = @'
    ...
    WHERE o.[is_ms_shipped] = 0
	    AND o.[type] IN ('V', 'U', 'P');
'@ | Invoke-SqlServerQuery @CommonQueryParameters
...
$SchemaBaseDir = Join-Path -Path "$($Path)" -ChildPath $SchemaName
...
"@
    ...
    $null = ($query -split '\n\s*GO\s*\n') | Foreach-Object { Invoke-SqlServerQuery @CommonQueryParameters -Verbose -CUD -Sql $_ }
}
...
foreach ($view in (Get-ChildItem -ErrorAction Ignore -LiteralPath (Join-Path -Path $SchemaBaseDir -ChildPath 'Views') -File -Filter '*.sql')) {
    ...
    $null = Invoke-SqlServerQuery @CommonQueryParameters -CUD -Sql $query -Verbose
}
