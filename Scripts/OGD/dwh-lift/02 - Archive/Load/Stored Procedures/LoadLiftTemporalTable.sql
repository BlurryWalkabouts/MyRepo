CREATE PROCEDURE [Load].[LoadLiftTemporalTable]
(
...
DECLARE @columnset table (column_name sysname);
...
SELECT
    arc.COLUMN_NAME
FROM
    INFORMATION_SCHEMA.COLUMNS arc
    INNER JOIN INFORMATION_SCHEMA.COLUMNS stg
    ON stg.TABLE_NAME = arc.TABLE_NAME AND stg.COLUMN_NAME = arc.COLUMN_NAME
WHERE 1=1
    AND arc.TABLE_SCHEMA = @archive_schema
    AND stg.TABLE_SCHEMA = @staging_schema
    AND arc.TABLE_NAME = @table
    AND arc.COLUMN_NAME NOT IN ('ValidFrom', 'ValidTo');

SELECT @select += CONCAT(QUOTENAME(column_name), ',')
FROM @columnset
WHERE column_name NOT IN ('AuditDWKey');
...
SET @SQLString = '
	...
	FROM
		' + @2partTableName + ' T
		JOIN ' + @staging_schema + '.' + @table + ' I ON I.' + @pkfield1 + ' = T.' + @pkfield1 + ' AND I.AuditDWKey = ' + CAST(@AuditDWKey as varchar(10)) + '
...
SET @msg = CONCAT('Finished loading Table ', @table, ' ', @AuditDWKey, ', ')
