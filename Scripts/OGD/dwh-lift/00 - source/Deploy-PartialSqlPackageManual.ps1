...
try {
    $ExistingSchemas = (Invoke-SqlServerQuery @commonQueryParameters -ErrorAction Stop `
...
"@
    ...
    $null = ($query -split '\n\s*GO\s*\n') | Foreach-Object { Invoke-SqlServerQuery @commonQueryParameters -CUD -Sql $_ }
}
...
foreach ($view in (Get-ChildItem -ErrorAction Ignore -LiteralPath (Join-Path -Path $SchemaBaseDir -ChildPath 'Views') -File -Filter '*.sql')) {
    ...
    $null = Invoke-SqlServerQuery @commonQueryParameters -CUD -Sql $query
}
