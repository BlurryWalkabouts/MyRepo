CREATE VIEW [Fin].[VwExcelMaandrapportage]
AS
...
SELECT
	...
	, kstdrcode = CASE kd.KostendragerCode WHEN '*****' THEN '0' ELSE kd.KostendragerCode END
	, kstdromschrijving = kd.KostendragerNaam
	, aantal = NULL
	, koers = NULL
	, bdr_val = m.MutatieBedrag
	, bud_vers = bs.ScenarioCode
	, TransactionType = tt.TransactieTypeCode
	, Rate = NULL
	, AmountCentral = m.MutatieBedrag
...
FROM
	...
	LEFT OUTER JOIN Dim.Kostendrager kd ON m.KostendragerKey = kd.KostendragerKey
